...
from simple_strategy import FIRST_TIER, SECOND_TIER, split_tiers
...
(function () {
  ...
  function renderCards(targetId, rows, tier) {
    ...
    if (!rows.length) {
      target.appendChild(node("div", "empty", tier === "first" ? "今天暂时没有第一梯队股票。" : "今天暂时没有第二梯队股票。"));
      return;
    }
    rows.forEach(function (item) {
      ...
      if (tier === "second") values.appendChild(node("span", "line-chip yellow", "黄 " + number(item.yellow_line_value)));
  ...
  function updateToday() {
    ...
    document.getElementById("today-total-value").textContent = String(current.first.length + current.second.length);
  ...
  function renderDayDetail(day) {
    ...
    if (!day) {
      detail.appendChild(node("div", "detail-empty", "选择一个有记录的日期，查看当天两梯队股票和它们的至今收益。"));
      return;
    }
    var first = day.first || [];
    var second = day.second || [];
    var head = node("div", "detail-date");
    head.append(node("h2", "", day.trade_date), node("span", "", "共 " + (first.length + second.length) + " 只"));
    ...
    detail.append(groupFirst, groupSecond);
  }
  function renderCalendar() {
    ...
    for (var dayNumber = 1; dayNumber <= totalDays; dayNumber += 1) {
      ...
      if (record) {
        ...
        button.append(dots, node("span", "count", String(first.length + second.length)));
        button.setAttribute("aria-label", key + "，共 " + (first.length + second.length) + " 只");
...
def render_report(
    ...
    title = f"每日两梯队选股 · {trade_date or '等待数据'}"
...
<head>
  ...
  <meta name="description" content="每日第一梯队、第二梯队选股与历史收益日历">
...
<body>
  <header class="topbar">
    <div class="shell topbar-inner">
      <div class="brand"><span class="brand-mark" aria-hidden="true"></span><span>每日两梯队选股</span></div>
  ...
  <main class="shell">
    <section id="today-view" class="view-panel">
      ...
      <section class="tier-section first">
        <div class="tier-title-row"><div><div class="tier-kicker"><span class="tier-index">01</span>优先查看</div><h2>第一梯队 <span id="first-count">0</span></h2></div><p class="tier-rule">近 2 个交易日出现可能见底，且此前连续 3 个交易日的龙虎线差值绝对值，每一天都不大于 0.5。</p></div>
